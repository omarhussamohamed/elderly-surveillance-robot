 ...
 * Optimizations:
 * - Dynamic yaw covariance for skid-steer slip compensation (high during rotation)
 * - Midpoint theta integration (Runge-Kutta 2) for accurate pose without circular drift
 * - Dead-zone velocity threshold to eliminate stationary noise
 * - Safety motor timeout
 * - Robust reconnection logic with serial debug
 * - Publish to /wheel_odom with frame_id "wheel_odom_frame" to match EKF config
 * - No TF broadcaster (EKF handles odom -> base_footprint)
 * - Covariances set for EKF fusion reliability
 * - Encoder polarity ensured positive forward on all wheels
 */
...
#define RECONNECT_INTERVAL 1000 // 1 second between reconnect attempts
...
void publishOdometry() {
  ...
  // Full covariance array initialization (all off-diagonals 0)
  for (int i = 0; i < 36; i++) {
  ...
  // Dynamic yaw covariance
  if (abs_linear < 0.05 && abs_angular > 0.1) {
    odom_msg.pose.covariance[35] = 1.0;  // yaw - high during pure rotation
  } else if (abs_linear < 0.1) {
    odom_msg.pose.covariance[35] = 0.5;  // high during slow motion
  } else {
    odom_msg.pose.covariance[35] = 0.05; // normal
  }
  
  // Twist covariance diagonals
  odom_msg.twist.covariance[0] = 0.002;   // vx
  odom_msg.twist.covariance[7] = 0.002;   // vy
  ...
  if (abs_linear < 0.05 && abs_angular > 0.1) {
    odom_msg.twist.covariance[35] = 2.0;  // vyaw - very high during pure rotation
  } else {
    odom_msg.twist.covariance[35] = 0.5;  // high normal
  }
...
void loop() {
  ...
  if (ros_ip_valid) {
    // Connection management with debug
    if (!ros_wifi_hw.connected()) {
      if (now - last_ros_connect > RECONNECT_INTERVAL) {
      ...
      if (now - last_ros_connect > 5000) {  // Log connected every 5s
    ...
    if (now - last_odom_time >= ODOM_PUBLISH_INTERVAL) {
      updateOdometry((now - last_odom_time) / 1000.0f);
      publishOdometry();
      last_odom_time = now;
    }
  }
